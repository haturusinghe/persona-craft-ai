...
from persona_craft_ai import domain
from persona_craft_ai.application import utils
from persona_craft_ai.domain.cleaned_documents import CleanedDocument
from persona_craft_ai.domain.dataset import DatasetType, TrainTestSplit
from persona_craft_ai.domain.prompt import GenerateDatasetSamplesPrompt, Prompt
...
Example instruction: Explain what clustering is used for in the context of Data Science. \
Example answer: Labeling data is not always an easy task. There are occasion that the data in hand does not have label and \
you need to make a model using them. You have to find the similarities and differences in your input data. \
Clustering approaches try to find these similarities and differences to find similar data. \
Also they are used as a pre-processing before doing supervised classification. \
In cases that the input data does not have any label, employing clustering approaches can be a way to label the data and \
use them for training supervised models. \
